...
| C6 | CI does not run the contract validations or the version check, `verify` omits the dynamic security smoke, the coherence check skips Kinova, and the workflow has no `permissions` block. | `.github/workflows/ci.yml`, `scripts/*` | Fixed |
