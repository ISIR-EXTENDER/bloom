...
describe("teleop targets the server refuses", () => {
  it("lists the app's topics the server's own list leaves out", () => {
    expect(
      teleopTargetsTheServerRefuses(["/joystick_cartesian_command", "/tablet_cmd"], ["/joystick_cartesian_command"]),
    ).toEqual(["/tablet_cmd"]);
  });
  ...
  it("says nothing while the server's list is unknown or open, or for the app's wildcard", () => {
    ...
    expect(teleopTargetsTheServerRefuses(["*"], ["/joystick_cartesian_command"])).toEqual([]);
  });
});
