...
async function startRosProbe() {
  ...
  const ready = await Promise.race([
    probe.waitFor(POSE, () => true, { timeoutMs: 30000 }).then(() => true),
    exited.then(() => false),
  ]).catch(() => false);
  if (!ready) {
    child.kill("SIGKILL");
    console.error(`No ${POSE} on ROS_DOMAIN_ID=${process.env.ROS_DOMAIN_ID ?? 0}; is ROS sourced and the robot up?`);
    process.exit(1);
  }
  // Discovery of the speed topic's subscribers lags the first samples.
